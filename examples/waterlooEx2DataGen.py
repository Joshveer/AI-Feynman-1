...
rho = np.random.uniform(1.0, 10.0, n) # Fluid density between 1.0 and 1.5 kg/m³
Cd = np.random.uniform(0.1, 10.0, n)  # Drag coefficient between 0.1 and 1.0
A = np.random.uniform(0.01, 10.0, n)    # Cross-sectional area between 0.01 and 1 m²

# Calculate terminal velocity (vt) using the equation vt = sqrt((2mg)/(ρCdA))
# Add a small epsilon to avoid division by zero
vt = ((2 * m * g) ** 0.5)/ (((rho * Cd * A) ** 0.5))
